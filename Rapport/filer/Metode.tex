...
Funktionen \texttt{threshold\_image} thresholder \textbf{(er det et fordansket ord?)} billedet med OpenCV funktionen \texttt{cv2.threshold} og udfører morfologiske operationer på det.
...
while not rospy.is_shutdown():
        ...
        rate.sleep()
\end{lstlisting}
Funktionen \texttt{find\_brick\_centers()} kaldes, \textbf{ogd!!} returnet x,y kooridnater på alle klodserne, der er i arbejdsområdet, i en liste.
Derefter bliver \textbf{første og anden pladsen!!!! kunne godt omskrives?} i listen, som er x og y koordinater på den første klods, lagt ind i en string og derefter publisheret.
Hvis der ikke er nogle klodser i arbejdsområdet bliver der sendt en tom string.

%%%%%%%%%% Robot system %%%%%%%%%%
\newpage
\section{Robot system}
Skriv indledende tekst... - er der nogen som har tænkt over dette eller? 
...
Robotten skal kunne bevæge sig hen til en given position ud fra et koordinatsæt. Da motorerne på crustcrawleren styres ud fra vinkler skal en matematisk konventering fra koordinatsæt til vinkler foretages.\newline
