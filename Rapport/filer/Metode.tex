...
%%%%%%%%%% System system %%%%%%%%%%
\section{Systemflow}
CrustCrawleren starter med at gå til en defineret udgangsposition hvor den står lodret.
Derefter tages et billede med webcamet, som er monteret ca. 1 meter over bordet som CrustCrawleren er monteret på.
Vision systemet udføres på billedet og finder centrum på klodserne på billedet inden for arbejdsområdet.
Koordinaterne blevet sendt til CrustCrawleren, hvorefter den bevæger sig til denne position.
...
Så snart trykket når grænseværdien bevæger robotten sig til spejlingspositionen i dubklikeringsområdet og åbner grabberen.
Dette fortsætter indtil der ikke findes flere klodser i arbejdsområdet.
Når CrustCrawleren ikke registrerer flere klodser bevæger den sig til en håndtryksposition, hvor den afventer en tryk i grabberen hvorefter programmet afsluttes.
Figur \vref{fig:Flowchart} viser et flowchart diagram over flowet i main funktionen.
...
derefter Subscribes på VisionPublisher og grabber\_check\_pressure, så programmet får værdierne af de 2 topics.
Derefter 
...
\textbf{Måske noget med frihedsgrader?}
...
\textbf{Bilag med Denavit-Hartenberg teori?}
...
Klassen \texttt{RobotExecute()} håndterer konstruktionen og afsendelsen af kommandoer til robotten. Dette gøres vha. ROSPY bibliotekets \texttt{SimpleActionClient}.

Først opbygges en liste af joint positions, der hver indeholde det enkelte joints position, en velocity samt tiden, der går fra start af kommandoen.
Derefter lægges disse positioner ind i et joinTrajectory objekt, der udover disse indeholder navnene, der beskriver de enkelte joints).
